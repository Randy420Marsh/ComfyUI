...
# -----------------------------------------------------------------------------
# uv-update-nodes_v3-no_install.sh (updated)
#
...
# - Updates existing repos with git pull (now default)
# - Skips all pip installs to avoid messing with dependencies
# - Skips pip check
# -----------------------------------------------------------------------------
...
usage() {
  cat <<'EOF'
Usage:
  ./uv-update-nodes_v3-no_install.sh [options]
...
Options:
  ...
  --python VERSION            uv python pin VERSION (e.g., 3.11). Default: 3.11
...
Examples:
  ...
  ./uv-update-nodes_v3.sh --html ./CustomNodeRepositories.html
EOF
}
...
DEPTH="1"
UPDATE_EXISTING="1"  # Now default to update existing with git pull
...
# -----------------------------------------------------------------------------
# Venv bootstrapping (for consistent Python version in parsing)
# -----------------------------------------------------------------------------
if [[ "${DRY_RUN}" == "0" ]]; then
